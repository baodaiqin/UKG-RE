	...
	def __init__(self, is_training, word_embeddings, max_length,
		...
		self.neg_r = tf.placeholder(tf.int32, [None])
			...
			temp_posi1_embedding = tf.get_variable('temp_posi1_embedding',[self.posi_num, self.posi_size],dtype=tf.float32,
                                                               initializer=tf.contrib.layers.xavier_initializer(seed=self.seed))
			temp_posi2_embedding = tf.get_variable('temp_posi2_embedding',[self.posi_num, self.posi_size],dtype=tf.float32,
                                                               initializer=tf.contrib.layers.xavier_initializer(seed=self.seed))
			self.posi1_embedding = tf.concat([temp_posi1_embedding,tf.reshape(tf.constant(np.zeros(self.posi_size,dtype=np.float32)),[1, self.posi_size])],0)
			self.posi2_embedding = tf.concat([temp_posi2_embedding,tf.reshape(tf.constant(np.zeros(self.posi_size,dtype=np.float32)),[1, self.posi_size])],0)

                        # relation embeddings and the transfer matrix between relations and textual relations
			self.rel_embeddings = tf.get_variable(name = "rel_embedding", shape = [self.rel_total, self.word_size],
                                                              initializer = tf.contrib.layers.xavier_initializer(seed=self.seed))
...
class CNN(NN):
	...
	def __init__(self, is_training, word_embeddings, max_length,
		...
		with tf.name_scope("conv-maxpool"):
			...
			x = tf.layers.conv2d(inputs = input_path, filters=self.hidden_size, kernel_size=[1,3], strides=[1, 1], padding='same',
