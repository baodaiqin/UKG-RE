...
import network_ug_plm as network
from kg_dataset_transe_plm import KnowledgeGraph
...
class Model:
    ...
    def __init__(self, dir_train=conf.DIR_TRAIN, dir_test=conf.DIR_TEST, model_dir=conf.MODEL_DIR,
        ...
        self.kgc_max_epoch = conf.PRE_MAX_EPOCH
        ...
        # plm
        self.tokenizer = transformers.BertTokenizer.from_pretrained(conf.BERT_NAME)
    ...
    def train(self):
        ...
        para_info = [['bags', len(self.data['instance_scope'])],
                     ['paths', len(self.data['train_input_ids'])],
        ...
        def pre_train_kgc():
            ...
            times_kg = 0
            pbar = tqdm(range(self.kgc_max_epoch), desc="KGC pre-training progress")
        ...
        def train_nn(coord):
            ...
            if strategy in ['none', 'pretrain', 'pretrain_ranking', 'locloss']:
                ...
                for one_epoch in pbar:
                    np.random.shuffle(list(train_order))
                    s1 = 0.0
                    s2 = 0.0
                    tot1 = 0.0
                    tot2 = 1.0
                    ...
                    for i in range(int(len(train_order)/float(self.batch_size))):
                        [label, weights, scope, scope_kg, scope_tx, scope_hy,
                         ...
                         train_input_ids_3tp, train_att_mask_3tp, train_ht_ids_3tp,
                        ...
                        if strategy in ['none']:
                            output, loss, correct_predictions = self.__train_step(sess, model, train_op, global_step,
                                                                                  train_head_batch, train_tail_batch, self.data['train_input_ids'][index,:],
                                                                                  self.data['train_att_mask'][index,:], self.data['train_ht_ids'][index,:],
                                                                                  self.data['train_label'][index],
                                                                                  label_, np.array(scope), weights, self.data['train_comp_fea'][index,:])

                        elif strategy in ['locloss']:
                            output, loss, correct_predictions = self.__train_step(sess, model, train_op, global_step,
                                                                                  train_head_3tp_batch, train_tail_3tp_batch, train_input_ids_3tp,
                                                                                  train_att_mask_3tp, train_ht_ids_3tp,
                                                                                  train_label_3tp,
                                                                                  label_, np.array(scope_3tp), weights, train_comp_fea_3tp)
                            
                        elif strategy in ['pretrain', 'pretrain_ranking']:
                            output, loss, correct_predictions = self.__train_step(sess, model, train_op, global_step,
                                                                                  train_head_tx_batch, train_tail_tx_batch, self.data['train_input_ids_tx'][index_tx,:],
                                                                                  self.data['train_att_mask_tx'][index_tx,:], self.data['train_ht_ids_tx'][index_tx,:],
                                                                                  self.data['train_label_tx'][index_tx],
                                                                                  label_, np.array(scope_tx), weights, self.data['train_comp_fea_tx'][index_tx,:])
                            
                            output, loss, correct_predictions = self.__train_step(sess, model, train_op, global_step,
                                                                                  train_head_hy_batch, train_tail_hy_batch, self.data['train_input_ids_hy'][index_hy,:],
                                                                                  self.data['train_att_mask_hy'][index_hy,:], self.data['train_ht_ids_hy'][index_hy,:],
                                                                                  self.data['train_label_hy'][index_hy],
                                                                                  label_, np.array(scope_hy), weights, self.data['train_comp_fea_hy'][index_hy,:])
                            
                            output, loss, correct_predictions = self.__train_step(sess, model, train_op, global_step,
                                                                                  train_head_kg_batch, train_tail_kg_batch, self.data['train_input_ids_kg'][index_kg,:],
                                                                                  self.data['train_att_mask_kg'][index_kg,:], self.data['train_ht_ids_kg'][index_kg,:],
        ...
        # pre-training KGC model.
        pre_train_kgc()
        # jointly training KGC model.
    ...
    def infer(self, lst_ep, nb_path, cutoff, model_dir):
        ...
        paths_head, paths_tail, paths_input_ids, paths_att_mask, paths_ht_ids, paths_scope, paths_comp_fea, lst_ep, ep2paths = self.__preprocess_ug_paths(bags)

        word_vec = np.load(self.dir_train + '/' + 'vec.npy')
        model = network.PLM(is_training=False, word_embeddings=word_vec, max_length=self.max_length,
        ...
        checkpoint_fle = tf.compat.v1.train.latest_checkpoint(model_dir)
        ...
        feed_dict = {
            ...
            model.input_ids: paths_input_ids,
            model.att_mask: paths_att_mask,
            model.ht_ids: paths_ht_ids,
    ...
    def test(self):
        ...
        model = network.PLM(is_training=False, word_embeddings=self.data_test['word_vec'], max_length=self.max_length,
                            num_classes=self.num_classes, hidden_size=self.hidden_size, posi_size=self.posi_size,
                            margin=self.margin, rel_total=self.rel_total, seed=self.seed,
                            batch_size=self.batch_size, testing_batch_size=self.testing_batch_size, rank_topn=self.rank_topn, strategy=self.strategy)
        ...
        def test_step(head, tail, input_ids, att_mask, ht_ids, label_index, label, scope, comp_fea):
            feed_dict = {
                model.head_index: head,
                model.tail_index: tail,
                model.input_ids: input_ids,
                model.att_mask: att_mask,
                model.ht_ids: ht_ids,
        ...
        iteration = int(len(instance_scope)/self.testing_batch_size)
        for i in range(iteration):
            ...
            output, test_att, test_pred = test_step(self.data_test["test_head"][index], self.data_test["test_tail"][index], self.data_test["test_input_ids"][index,:],
                                                    self.data_test["test_att_mask"][index,:], self.data_test["test_ht_ids"][index,:], self.data_test["test_label"][index],
    ...
    def __train_step(self, sess, model, train_op, global_step, head, tail, input_ids, att_mask, ht_ids, label_index, label, scope, weights, comp_fea):
        feed_dict = {
            model.head_index: head,
            model.tail_index: tail,
            model.input_ids: input_ids,
    ...
    def __load_data(self, is_training=True):
        ...
        data['%s_input_ids' % tag] = np.load(export_path + 'input_ids.npy')
        data['%s_att_mask' % tag] = np.load(export_path + 'att_mask.npy')
        data['%s_ht_ids' % tag] = np.load(export_path + 'ht_ids.npy')

        data['%s_input_ids_kg' % tag] = np.load(export_path + 'input_ids_kg.npy')
        data['%s_att_mask_kg' % tag] = np.load(export_path + 'att_mask_kg.npy')
        data['%s_ht_ids_kg' % tag] = np.load(export_path + 'ht_ids_kg.npy')

        data['%s_input_ids_tx' % tag] = np.load(export_path + 'input_ids_tx.npy')
        data['%s_att_mask_tx' % tag] = np.load(export_path + 'att_mask_tx.npy')
        data['%s_ht_ids_tx' % tag] = np.load(export_path + 'ht_ids_tx.npy')

        data['%s_input_ids_hy' % tag] = np.load(export_path + 'input_ids_hy.npy')
        data['%s_att_mask_hy' % tag] = np.load(export_path + 'att_mask_hy.npy')
        data['%s_ht_ids_hy' % tag] = np.load(export_path + 'ht_ids_hy.npy')

        data['%s_head' % tag] = np.load(export_path + 'head.npy')
        data['%s_tail' % tag] = np.load(export_path + 'tail.npy')

        data['%s_comp_fea' % tag] = complexity_features(data['%s_input_ids' % tag])
        data['%s_comp_fea_kg' % tag] = complexity_features(data['%s_input_ids_kg' % tag])
        data['%s_comp_fea_tx' % tag] = complexity_features(data['%s_input_ids_tx' % tag])
        data['%s_comp_fea_hy' % tag] = complexity_features(data['%s_input_ids_hy' % tag])
    ...
    def __load_data_batch(self, i, train_order):
        ...
        for num, num_kg, num_tx, num_hy in zip(input_scope, input_scope_kg, input_scope_tx, input_scope_hy):
            ...
            index_kg = index_kg + list(range(num_kg[0], num_kg[1] + 1))
            scope_kg.append(scope_kg[len(scope_kg)-1] + num_kg[1] - num_kg[0] + 1)
            index_tx = index_tx + list(range(num_tx[0], num_tx[1] + 1))
            scope_tx.append(scope_tx[len(scope_tx)-1] + num_tx[1] - num_tx[0] + 1)
            index_hy = index_hy + list(range(num_hy[0], num_hy[1] + 1))
            ...
            train_input_ids_3tp.extend(self.data['train_input_ids_kg'][num_kg[0]:num_kg[1] + 1])
            train_input_ids_3tp.extend(self.data['train_input_ids_tx'][num_tx[0]:num_tx[1] + 1])
            train_input_ids_3tp.extend(self.data['train_input_ids_hy'][num_hy[0]:num_hy[1] + 1])
            train_att_mask_3tp.extend(self.data['train_att_mask_kg'][num_kg[0]:num_kg[1] + 1])
            train_att_mask_3tp.extend(self.data['train_att_mask_tx'][num_tx[0]:num_tx[1] + 1])
            train_att_mask_3tp.extend(self.data['train_att_mask_hy'][num_hy[0]:num_hy[1] + 1])
            train_ht_ids_3tp.extend(self.data['train_ht_ids_kg'][num_kg[0]:num_kg[1] + 1])
            train_ht_ids_3tp.extend(self.data['train_ht_ids_tx'][num_tx[0]:num_tx[1] + 1])
            train_ht_ids_3tp.extend(self.data['train_ht_ids_hy'][num_hy[0]:num_hy[1] + 1])
        ...
        train_input_ids_3tp = np.array(train_input_ids_3tp).astype(np.int32)
        train_att_mask_3tp = np.array(train_att_mask_3tp).astype(np.int32)
        train_ht_ids_3tp = np.array(train_ht_ids_3tp).astype(np.int32)
    ...
    def __model_init(self, strategy=None):
        if not strategy:
            ...
            strategy = strategy
        model = network.PLM(is_training=True, word_embeddings=self.data['word_vec'], max_length=self.max_length,
                            num_classes=self.num_classes, hidden_size=self.hidden_size, posi_size=self.posi_size,
                            margin=self.margin, rel_total=self.rel_total, seed=self.seed,
        ...
        print('building KGC ...')
        optimizer_kgc = tf.compat.v1.train.GradientDescentOptimizer(self.learning_rate_kgc)
    ...
    def __preprocess_ug_paths(self, bags):
        ...
        def indexing_paths(paths, posis, total):
            ...
            for ep, lst_path in pbar:
                ...
                for path, posi in zip(lst_path, lst_posi):
                    ...
                    path = ' '.join(path)
                    encoded_dict = self.tokenizer.encode_plus(path, max_length=fixlen, pad_to_max_length=True)
                    ...
                    toks = self.tokenizer.tokenize(path)
                    nb_tok = len(toks)
                    paths_ht_ids[pathi] = [1, nb_tok]
            return paths_head, paths_tail, paths_input_ids, paths_att_mask, paths_ht_ids, np.array(instance_scope), complexity_features(paths_input_ids), instance_ep
        ...
        paths_head, paths_tail, paths_input_ids, paths_att_mask, paths_ht_ids, instance_scope, paths_comp_fea, lst_ep = indexing_paths(paths, posis, total)
        scope = [0]
        for num in instance_scope:
            scope.append(scope[len(scope)-1] + num[1] - num[0] + 1)

        return paths_head, paths_tail, paths_input_ids, paths_att_mask, paths_ht_ids, np.array(scope), paths_comp_fea, lst_ep, paths
